    ...
    def setup_augmentors(self, augmentations):
        ...
        for aug_name, aug_config in augmentations.items():
            ...
            if aug_name == 'grayscale':
                ...
                aug = iaa.GaussianBlur(sigma=(aug_config.get(
                    'sigma_min', 0.0), aug_config.get('sigma_max', 0.0)))
            elif aug_name == 'defocus':
                aug = iaa.imgcorruptlike.DefocusBlur(severity=(aug_config.get(
                    'min_severity', 1), aug_config.get('min_severity', 3)))
            elif aug_name == 'fog':
                aug = iaa.imgcorruptlike.Fog(severity=(aug_config.get(
                    'min_severity', 1), aug_config.get('min_severity', 3)))
                ...
                aug = iaa.PerspectiveTransform(scale=(aug_config.get(
                    'min_scale', 0), aug_config.get('max_scale', 0.05)))
            elif aug_name == 'elastic_transform':
                # Note: Ground-truth seems to shift a bit, but imgaug does not implement it
                aug = ElasticTransformCorruption(severity=(aug_config.get(
                    'min_severity', 1), aug_config.get('min_severity', 3)))
...
class VariableRangeAdditiveGaussianNoise:
    ...
    def __call__(self, image, polygons=None):
        upper_scale = (random.random() * self.upper_range) + \
            self.min_upper_scale
